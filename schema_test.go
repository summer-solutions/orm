...
func TestSchema(t *testing.T) {
	...
	tableSchema.DropTable(engine)
	tableSchemaRef := validatedRegistry.GetTableSchema("orm.testEntitySchemaRef")
	tableSchemaRef.DropTable(engine)
	_ = tableSchemaRef.GetMysql(engine).Exec("DROP TABLE IF EXISTS `ToDrop`")
	logDB := engine.GetMysql("log")
	_ = logDB.Exec("DROP TABLE IF EXISTS `_log_schema_testEntitySchema`")
	_ = logDB.Exec("DROP TABLE IF EXISTS `_log_default_testEntityLog`")
	...
	_ = tableSchema.GetMysql(engine).Exec("CREATE TABLE `ToDrop` (`ID` int(10) unsigned NOT NULL) ENGINE=InnoDB DEFAULT CHARSET=utf8;")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	assert.Equal(t, "DROP TABLE IF EXISTS `test_schema`.`ToDrop`;", alters[0].SQL)

	_ = tableSchema.GetMysql(engine).Exec("INSERT INTO `ToDrop`(ID) VALUES(1)")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.False(t, alters[0].Safe)
	assert.Equal(t, "DROP TABLE IF EXISTS `test_schema`.`ToDrop`;", alters[0].SQL)

	_ = tableSchema.GetMysql(engine).Exec(alters[0].SQL)

	_ = tableSchema.GetMysql(engine).Exec("INSERT INTO `testEntitySchema`(Name) VALUES('test')")
	...
	assert.Len(t, columns, 35)
	assert.Equal(t, []string{"ID", "Uint8", "Uint24", "Uint32", "Uint64", "Year", "YearNotNull", "Int8", "Int16", "Int32", "Int32Medium", "Int64", "Rune", "Int", "Name", "NameNotNull", "BigName", "Enum", "EnumNotNull", "Set", "Blob", "MediumBlob", "Bool", "Float32", "Float64", "Float32Decimal", "Float64DecimalSigned", "Date", "DateTime", "DateNotNull", "JSON", "ReferenceOne", "ReferenceOneCascade", "AddressBuilding", "AddressStreet"}, columns)

	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` ADD COLUMN `ToDrop` int(8)")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	...
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` CHANGE COLUMN `Enum` `Enum` enum('Red','Black','Blue','Yellow','Purple') DEFAULT NULL")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	...
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` DROP INDEX `SecondIndex`")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	...
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` DROP INDEX `SecondIndex`, ADD UNIQUE INDEX `SecondIndex` (`Uint8`, `Uint64`);")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	...
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` DROP FOREIGN KEY `test_schema:testEntitySchema:ReferenceOne`")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	...
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` DROP FOREIGN KEY `test_schema:testEntitySchema:ReferenceOne`")
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` ADD CONSTRAINT `test_schema:testEntitySchema:ReferenceOne` FOREIGN KEY (`ReferenceOne`) REFERENCES `test_schema`.`testEntitySchemaRef` (`ID`) ON DELETE CASCADE;")
	alters = engine.GetAlters()
	...
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` ADD KEY `ToDropIndex` (`Uint8`)")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	...
	_ = tableSchema.GetMysql(engine).Exec("ALTER TABLE `testEntitySchema` ADD CONSTRAINT `ToDropConstrait` FOREIGN KEY (`ReferenceOne`) REFERENCES `testEntitySchemaRef` (`ID`)")
	alters = engine.GetAlters()
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
