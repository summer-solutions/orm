...
func TestSchema(t *testing.T) {
	...
	assert.Len(t, alters, 1)
	assert.True(t, alters[0].Safe)
	assert.Equal(t, "ALTER TABLE `test_schema`.`testEntitySchema`\n    CHANGE COLUMN `Enum` `Enum` enum('Blue','Green','Purple','Red','Yellow') DEFAULT NULL AFTER `Float64DecimalSigned`;/*CHANGED FROM `Enum` enum('Red','Black','Blue','Yellow','Purple') DEFAULT NULL*/", alters[0].SQL)
	err = tableSchema.UpdateSchema(engine)
	assert.Nil(t, err)
