...
func testRedis(t *testing.T, engine *Engine) {
	...
	id := r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a", "code", "b"}})
	...
	// ADD using pipeline
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a1", "code", "b1"}})
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a2", "code", "b2"}})
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a3", "code", "b3"}})
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a4", "code", "b4"}})
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a5", "code", "b5"}, MaxLen: 4})
	assert.Equal(t, int64(4), r.XLen("test-stream"))
	messages := r.XRange("test-stream", "-", "+", 10)
	assert.Len(t, messages, 4)
	assert.Equal(t, "a2", messages[0].Values["name"])
	assert.Equal(t, "a5", messages[3].Values["name"])
	assert.Equal(t, int64(1), r.XDel("test-stream", messages[1].ID))
	assert.Equal(t, int64(3), r.XLen("test-stream"))
	messages = r.XRevRange("test-stream", "+", "-", 2)
	...
	streams := r.XRead(&redis.XReadArgs{Streams: []string{"test-stream", "0"}})
	...
	streams = r.XRead(&redis.XReadArgs{Streams: []string{"test-stream", "0"}, Count: 2})
	assert.Len(t, streams, 1)
	assert.Len(t, streams[0].Messages, 2)
	assert.Equal(t, "a2", streams[0].Messages[0].Values["name"])
	streams = r.XRead(&redis.XReadArgs{Streams: []string{"test-stream", streams[0].Messages[1].ID},
		Block: time.Millisecond * 10})
	assert.Len(t, streams, 1)
	assert.Len(t, streams[0].Messages, 1)
	assert.Equal(t, "a5", streams[0].Messages[0].Values["name"])
	r.XTrim("test-stream", 0, true)
	streams = r.XRead(&redis.XReadArgs{Streams: []string{"test-stream", "0"}, Block: -1})
	...
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a3", "code", "b3"}})
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a4", "code", "b4"}})
	r.XAdd(&redis.XAddArgs{ID: "*", Stream: "test-stream", Values: []string{"name", "a5", "code", "b5"}})
