...
func TestRedis(t *testing.T) {
	...
	registry.RegisterRedisChannel("test-stream-4", "default", 4)
...
func testRedis(t *testing.T, engine *Engine) {
	...
	id := r.XAdd("test-stream", []string{"name", "a", "code", "b"})
	...
	// ADD using pipeline
	r.XAdd("test-stream-4", []string{"name", "a1", "code", "b1"})
	r.XAdd("test-stream-4", []string{"name", "a2", "code", "b2"})
	r.XAdd("test-stream-4", []string{"name", "a3", "code", "b3"})
	r.XAdd("test-stream-4", []string{"name", "a4", "code", "b4"})
	r.XAdd("test-stream-4", []string{"name", "a5", "code", "b5"})
	assert.Equal(t, int64(4), r.XLen("test-stream-4"))
	messages := r.XRange("test-stream-4", "-", "+", 10)
	assert.Len(t, messages, 4)
	assert.Equal(t, "a2", messages[0].Values["name"])
	assert.Equal(t, "a5", messages[3].Values["name"])
	assert.Equal(t, int64(1), r.XDel("test-stream-4", messages[1].ID))
	assert.Equal(t, int64(3), r.XLen("test-stream-4"))
	messages = r.XRevRange("test-stream-4", "+", "-", 2)
	assert.Len(t, messages, 2)
	assert.Equal(t, "a5", messages[0].Values["name"])
	assert.Equal(t, "a4", messages[1].Values["name"])
	info := r.XInfoStream("test-stream-4")
	...
	assert.Equal(t, "a2", info.FirstEntry.Values["name"])
	assert.Equal(t, "a5", info.LastEntry.Values["name"])
	streams := r.XRead(&redis.XReadArgs{Streams: []string{"test-stream-4", "0"}})
	assert.Len(t, streams, 1)
	assert.Equal(t, "test-stream-4", streams[0].Stream)
	assert.Len(t, streams[0].Messages, 3)
	assert.Equal(t, "a2", streams[0].Messages[0].Values["name"])
	streams = r.XRead(&redis.XReadArgs{Streams: []string{"test-stream-4", "0"}, Count: 2})
	assert.Len(t, streams, 1)
	assert.Len(t, streams[0].Messages, 2)
	assert.Equal(t, "a2", streams[0].Messages[0].Values["name"])
	streams = r.XRead(&redis.XReadArgs{Streams: []string{"test-stream-4", streams[0].Messages[1].ID},
		Block: time.Millisecond * 10})
	assert.Len(t, streams, 1)
	assert.Len(t, streams[0].Messages, 1)
	assert.Equal(t, "a5", streams[0].Messages[0].Values["name"])
	r.XTrim("test-stream-4", 0, true)
	streams = r.XRead(&redis.XReadArgs{Streams: []string{"test-stream-4", "0"}, Block: -1})
	assert.Nil(t, streams)

	r.XAdd("test-stream", []string{"name", "a1", "code", "b1"})
	r.XAdd("test-stream", []string{"name", "a2", "code", "b2"})
	r.XAdd("test-stream", []string{"name", "a3", "code", "b3"})
	r.XAdd("test-stream", []string{"name", "a4", "code", "b4"})
	r.XAdd("test-stream", []string{"name", "a5", "code", "b5"})
	...
	r.XAdd("test-stream-a", []string{"name", "a1"})
	r.XAdd("test-stream-b", []string{"name", "b1"})
